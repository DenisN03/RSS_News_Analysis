...
def create_data_mart_category():
    ...
    cursor.execute(f"CREATE TABLE IF NOT EXISTS data_mart_category ("
    ...
    for name in urls_dict.keys():
        cursor.execute(f"ALTER TABLE data_mart_category ADD COLUMN IF NOT EXISTS DMC{name}NewsTotalCount INTEGER")
        cursor.execute(f"ALTER TABLE data_mart_category ADD COLUMN IF NOT EXISTS DMC{name}NewsDailyCount INTEGER")
...
def fill_data_mart_category():
    ...
    for category_id, category in categories:
        ...
        # Проход по всем источникам данных
        for name in urls_dict.keys():

            # Подсчет количества новостей в данной категории за все время
            ...
            # Подсчет количества новостей в данной категории за последние сутки
            cursor.execute(f"SELECT COUNT(*) FROM {name}_news nws "
                           f"WHERE nws.newscategory IN "
                           f"( SELECT ct.categoryname FROM {name}_categories ct "
                           f"WHERE ct.categoryid = {category_id} ) "
            ...
            # Вычисление среднего количества новостей по данной категории в сутки
            ...
            # Вычисление даты с максимальным количеством новостей по данной категории
            cursor.execute(
                f"SELECT DATE_TRUNC('day', to_timestamp(nws.newspubdate)::timestamptz at time zone 'UTC') AS day, COUNT(*) AS cnt "
                f"FROM {name}_news nws "
                f"WHERE nws.newscategory "
            ...
            # Вычисление количества новостей по дням недели для данной категории
            cursor.execute(
    ...
    cursor.execute("TRUNCATE TABLE data_mart_category")

    # Заполнение data mart подготовленными данными
    for _, query in query_data.items():
        cursor.execute(
            "INSERT INTO data_mart_category (" + ("{}, " * len(query.keys())).format(*query.keys())[:-2] +
